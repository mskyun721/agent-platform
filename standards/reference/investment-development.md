...
- 외부 연구(논문·리포트)를 근거로 쓰면 인용한 수치 옆에 평가 구간·표본 수·기준선·비용
  가정을 함께 적는다. 이것들이 없으면 방향성 근거로만 쓰고 수치 근거로 쓰지 않는다.
- 같은 문헌의 본문·표 수치가 다르면 단위·평가 범위·각주·문서 버전·정정본을 확인한다.
  해결되지 않으면 양쪽 값과 페이지를 기록하고 불일치로 표시하며 어느 쪽도 자동 우선하지 않는다.
- 원문을 직접 확인하지 않고 요약·검색 결과만 본 수치는 2차 출처로 표시한다.
