...
토큰 점검 요청은 `observe efficiency --root <explicit-root>`로 기존 관측 표본을
먼저 확인한다. 입력 사용량이 큰 실행은 조사 후보이며 낭비로 단정하지 않는다.
코드 관계는 graphify, 맥락이 부족하면 iterative-retrieval을 선택하고 필요한 파일만
추가한다. 모든 역할·스킬을 한꺼번에 읽거나 호출하지 않는다. skill-stocktake는
스킬 정리 요청에만 사용한다. 테스트·보안·승인 조건은 그대로 유지한다.
절감 효과는 같은 업무와 평가 조건의 품질·사용량·사용자 개입을 함께 비교해야 한다.
설정돼 있다는 사실을 실제 스킬 수행·성공 증거로 기록하지 않는다.
