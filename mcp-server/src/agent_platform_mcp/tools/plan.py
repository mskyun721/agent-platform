"""Planning wrapper — delegates PRD/TASK generation to Gemini CLI."""
...
def _frontmatter(feature: str, action: str) -> str:
    ...
    for fname in ([PRD_FILE, TASK_FILE] if action == "all" else [PRD_FILE if action == "prd" else TASK_FILE]):
        ...
        if fpath.is_file():
            ...
            if not content.lstrip().startswith("---"):
                fpath.write_text(_frontmatter(feature, action) + content, encoding="utf-8")
